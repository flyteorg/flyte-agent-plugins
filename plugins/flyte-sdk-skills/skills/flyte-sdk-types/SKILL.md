...
| `flyte.File` | Blob | Uploaded to metadata bucket |
| `flyte.Directory` | Blob (directory) | Uploaded to metadata bucket |
| `flyte.DataFrame` | DataFrame | Parquet in metadata bucket |
...
## flyte.File — Single Files

Use `flyte.File` for any single file that flows between tasks. Files are **automatically uploaded** to the metadata bucket at runtime.

```python
import flyte

@env.task
async def download_url(url: str) -> flyte.File:
    """Download a file and return as flyte.File."""
    import urllib.request
    local_path = f"/tmp/{url.split('/')[-1]}"
    urllib.request.urlretrieve(url, local_path)
    return flyte.File(path=local_path)

@env.task
async def process(file: flyte.File) -> dict:
    """Read a file — flyte.File downloads it automatically."""
...
async def main(url: str) -> dict:
    ...
    return await process(downloaded)  # type: flyte.File flows as remote reference
```

### flyte.File best practices

- **Always pass `flyte.File` between tasks** — never pass file paths as strings. Flyte serializes the reference to the remote blob.
...
## flyte.Directory — Directories

Use `flyte.Directory` for a collection of files (e.g., model checkpoints, output artifacts).

```python
import flyte

@env.task
async def train(checkpoint_dir: flyte.Directory) -> flyte.Directory:
...
@env.task
async def evaluate(checkpoints: flyte.Directory) -> dict:
...
## flyte.DataFrame — Polars DataFrames
...
@env.task
async def load_csv(url: str) -> flyte.DataFrame:
...
@env.task
async def clean(df: flyte.DataFrame) -> flyte.DataFrame:
    """Clean the DataFrame."""
...
@env.task
async def save_parquet(df: flyte.DataFrame, path: str) -> flyte.File:
...
@env.task
async def main(url: str) -> flyte.DataFrame:
...
# Convert from Polars
df = flyte.DataFrame(inner_df)

# Common operations
df = flyte.DataFrame(inner_df.filter(pl.col("age") > 18))
df = flyte.DataFrame(inner_df.group_by("category").agg(pl.col("value").mean()))
...
df = flyte.DataFrame(inner_df.lazy())
...
@env.task
async def train(config: TrainingConfig) -> flyte.File:
    # config.learning_rate, config.batch_size, etc.
    ...

@env.task
async def predict(model: flyte.File, data: flyte.DataFrame) -> PredictionOutput:
...
# Now use it in tasks
@env.task
async def process_image(img: Image.Image) -> flyte.File:
...
@env.task(cache="auto")
async def csv_to_parquet(csv_file: flyte.File, output_path: str) -> flyte.File:
    import polars as pl
...
@env.task
async def resize_image(input_file: flyte.File, size: tuple[int, int]) -> flyte.File:
...
@env.task
async def batch_resize(files: list[flyte.File], size: tuple[int, int]) -> list:
    import asyncio
...
@env.task
async def extract_features(audio_file: flyte.File) -> flyte.File:
...
@env_task
async def load_dataset_from_hub(dataset_name: str, split: str = "train") -> flyte.File:
...
@env.task
async def process_dataset(file: flyte.File) -> flyte.DataFrame:
...
| `flyte.File` | Reference (S3/GCS) | Unlimited |
| `flyte.Directory` | Reference (S3/GCS) | Unlimited |
| `flyte.DataFrame` (Polars) | Reference (Parquet) | Unlimited |
| `dataclass` / `BaseModel` | Inline (JSON) | ~10 MB |

### Storage best practices

1. **Use `flyte.File` for files** — don't pass strings. Flyte manages the upload/download.
2. **Use `flyte.DataFrame` for tabular data** — stored as Parquet, efficient for ETL pipelines.
3. **Use `flyte.Directory` for collections** — model checkpoints, output artifacts.
...
6. **Large data should never be inline** — if your dataclass exceeds ~10 MB, switch to `flyte.File` or `flyte.Directory`.
...
2. **Passing `flyte.File` as a string** — always use `flyte.File(path=...)` objects. The path string alone won't serialize.
...
5. **Forgetting `.path` on flyte.File** — inside a task, `file` is a FlyteFile object, not a string. Use `file.path` for the local path.
