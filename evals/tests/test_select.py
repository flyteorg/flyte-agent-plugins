...
def test_single_skill_change_selects_only_that_skill():
    m = _manifest()
    scs = load_scenarios()
    out = select(["plugins/flyte/skills/flyte-sdk-author/SKILL.md"], m, scs)
...
def test_kind_skill_sets_run_kind():
    m = _manifest()
    scs = load_scenarios()
    out = select(["plugins/flyte/skills/deploy-flyte-kind/SKILL.md"], m, scs)
    assert out["run_kind"] is True
    out2 = select(["plugins/flyte/skills/flyte-sdk-types/SKILL.md"], m, scs)
...
def test_is_shared_infra():
    ...
    assert not is_shared_infra("plugins/flyte/skills/flyte-sdk-run/SKILL.md", m)
